# Formal Methods for Verification of mrrc
...
with different cost/benefit tradeoffs at each level. This document proposes a
practical verification strategy for mrrc that uses the right tool at each level
of the pyramid, distributed between the mrrc library and mrrc-testbed according
to where each method provides the most value.
...
## Appendix: Cross-Repository Execution Plan

Working across two repositories with dependency relationships requires
deliberate sequencing. This appendix describes the practical mechanics:
what happens in which repo, in what order, how work flows between them,
and how to manage the whole effort without it becoming unwieldy.
...
The six phases in the main document describe *what* to build. This section
describes *how* to sequence the work across the two repos day by day. The
phases do not need to be executed strictly serially — they are grouped into
three waves that can overlap.
...
Not everything in this proposal needs to happen. The value curve is steep
